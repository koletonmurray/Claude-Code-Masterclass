...
export const missionConverter = {
  toFirestore: (data: Partial<Mission>): DocumentData => data,

  fromFirestore: (snapshot: QueryDocumentSnapshot): Mission => ({
    id: snapshot.id,
    ...snapshot.data(),
...
// Usage
const ref = collection(db, COLLECTIONS.MISSIONS).withConverter(missionConverter)
```
